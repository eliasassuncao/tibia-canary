-- Extended Opcode para botão customizado
local customButton = CreatureEvent("CustomButtonOpcode")
customButton:type("extendedopcode")

function customButton.onExtendedOpcode(player, opcode, buffer)
    -- Opcode 1 = Abrir menu customizado
    if opcode == 1 then
...
-- Reutilizar as funções do menu avançado que você já criou
local function createMainMenu(player)
    local window = ModalWindow {
        title = "Menu Principal - Via Botão",
        message = "Menu aberto através do botão customizado!"
    }
